...
- Binary provenance at the `v1.2.0-tandem.1` tag: the Git tree contains 0 executable binaries and 0 Git LFS objects. `banner.png` (600×200 PNG) is that tag's only tracked non-text file. `dist/` is ignored and contains only local audit/rebuild outputs. At the time of the `.1` validation, the organization repository had 1 release: `v1.2.0-tandem.1`, whose 6 executable assets match the locally generated SHA-256 values in its seventh asset, `checksums.txt`.
